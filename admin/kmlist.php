...
if($_GET['my']=='rm')
{
$rs=$DB->query("SELECT * FROM msg_km WHERE 1");
while($res = $DB->fetch($rs))
{
$id = $res['id'];
$sql="DELETE FROM msg_km WHERE id='$id' limit 1";
$DB->query($sql);
}
exit(alert("清空完成"));
}
...
if($_GET['my']=='s')
{
$rs=$DB->query("SELECT * FROM msg_km WHERE status=1");
while($res = $DB->fetch($rs))
{
$id = $res['id'];
$sql="DELETE FROM msg_km WHERE id='$id' limit 1";
...
if($_GET['my']=='del'){
	...
	$gls=$DB->count("SELECT count(*) from msg_km WHERE 1");
	$sql=" 1";
...
$ok=$DB->count("SELECT count(*) from msg_km WHERE status=0");
$no=$DB->count("SELECT count(*) from msg_km WHERE status=1");
...
?>
      <div class="table-responsive">
        <table class="table table-striped">
          ...
          <tbody>
<?php
$rs=$DB->query("SELECT * FROM msg_km WHERE{$sql} order by id desc limit $pageu,$pagesize");
while($res = $DB->fetch($rs))
{
