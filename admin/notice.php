...
if($my == "edit"){
$id=intval($_GET['id']);
$row=$DB->get_row("select * from msg_notice where id='$id' limit 1");
...
if($DB->query("update msg_notice set center='$center' where id='{$id}'"))
...
<tbody>
<?php
$rs=$DB->query("SELECT * FROM msg_notice WHERE 1 order by id asc");
...
$DB->query("insert into `msg_notice` (`center`,`date`) values ('".$center."','".$date."')");
}
}elseif($my == "del_submit"){
$id = $_GET['id'];

$sql="alter table `msg_notice` AUTO_INCREMENT=1";
$DB->query("DELETE FROM msg_notice WHERE id='$id'");
if($DB->query($sql))
